...
if (Meteor.isClient) {
  _.extend(MeteorFile.prototype, {
    ...
    upload: function (file, method, options, callback) {
      ...
      if (!Blob.prototype.isPrototypeOf(file)) {
        throw new Meteor.Error(
          "First parameter must inherit from Blob");
      }
      if (!_.isString(method)) {
        throw new Meteor.Error(
          "Second parameter must be a Meteor.method name");
      }
        ...
        if (self.bytesUploaded < self.size) {
          self.read(file, options, function (err, res) {
            if (err) {
              ...
              //console.log([self].concat(options.params || []));
              Meteor.apply(
                //method, [self].concat(options.params || []), {
                method, [self].concat(options || []), {
                  wait: true
                },
                function (err) {
                  if (err) {
    ...
    _setStatus: function (err) {
      ...
      this.uploadProgress = Math.
      round(this.bytesUploaded / this.size * 100);
  ...
  _.extend(MeteorFile, {
    ...
    upload: function (file, method, options, callback) {
      return new MeteorFile(file).upload(file, method, options,
        callback);
    }
  });
}
...
if (Meteor.isServer) {
  ...
  _.extend(MeteorFile.prototype, {
    save: function (dirPath, options) {
      // should do dirPath checking here
      //(see the next video or two in the series)
      var fileName = this.name;
      if (options.fileName) {
        fileName = options.fileName;
      }
      var filepath = path.join(dirPath, sanitize(fileName));
      var buffer = new Buffer(this.data);
      console.log("file size: " + this.data.length);
